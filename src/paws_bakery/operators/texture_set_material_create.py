...
def _select_base_template(bake_types: Set[str], user_preference: str = "") -> str:
    ...
    complex_types = {BakeTextureType.AO.name, BakeTextureType.EMIT.name, 
                    BakeTextureType.AORM.name, BakeTextureType.SHADOW.name}
...
def _load_material_template(template_name: str) -> b_t.Material:
    ...
    try:
        with bpy.data.libraries.load(str(UTIL_MATS_PATH), link=False, assets_only=True) as (data_src, data_dst):
            if template_name not in data_src.materials:
                raise AddonException(f"Template material '{template_name}' not found in materials.blend")
        ...
        raise AddonException(f"Failed to load material template '{template_name}': {str(e)}")


def _find_texture_nodes_by_prefix(material: b_t.Material, tex_alias: TextureTypeAlias) -> List[b_t.ShaderNodeTexImage]:
    ...
    log(f"Found {len(matching_nodes)} nodes with prefix '{prefix}': {[n.name for n in matching_nodes]}")
    return matching_nodes


def _ensure_texture_node_exists(material: b_t.Material, tex_alias: TextureTypeAlias) -> List[b_t.ShaderNodeTexImage]:
...
def _cleanup_unused_texture_nodes(material: b_t.Material, used_texture_aliases: Set[TextureTypeAlias]):
    ...
    log(f"Removing {len(nodes_to_remove)} unused texture nodes: {[n.name for n in nodes_to_remove]}")
    for node in nodes_to_remove:
        material.node_tree.nodes.remove(node)


def _assign_textures_to_nodes(material: b_t.Material, texture_images: Dict[str, b_t.Image]):
    ...
    for bake_type, image in texture_images.items():
        ...
        if not texture_nodes:
            log(f"No texture nodes found for {tex_alias.node_name}, skipping {bake_type}")
            continue
...
class TextureSetMaterialCreate(b_t.Operator):
    ...
    def execute(self, context: b_t.Context) -> set[str]:
        ...
        log(f"TextureSetMaterialCreate.execute() - texture_set_id: {self.texture_set_id}")
