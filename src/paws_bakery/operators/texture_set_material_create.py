...
def _get_texture_type_socket_info(texture_type: str) -> dict[str, str] | None:
    ...
    print(
        f"DEBUG: _get_texture_type_socket_info called with texture_type: '{texture_type}'"
    )
...
def _find_principled_bsdf(
    material: b_t.Material,
) -> b_t.ShaderNodeBsdfPrincipled | None:
    """Find the Principled BSDF node in material."""
    print(
        f"DEBUG: _find_principled_bsdf called with material: {material.name if material else None}"
    )
...
def _handle_aorm_texture(
    node_tree,
    tex_node: b_t.ShaderNodeTexImage,
    principled_bsdf: b_t.ShaderNodeBsdfPrincipled,
) -> None:
    ...
    try:
        ...
        if (
            "Roughness" in principled_bsdf.inputs
            and not principled_bsdf.inputs["Roughness"].is_linked
        ):
            node_tree.links.new(
                separate_rgb.outputs["G"], principled_bsdf.inputs["Roughness"]
            )
            print(f"DEBUG: Connected G channel to Roughness")

        if (
            "Metallic" in principled_bsdf.inputs
            and not principled_bsdf.inputs["Metallic"].is_linked
        ):
            node_tree.links.new(
                separate_rgb.outputs["B"], principled_bsdf.inputs["Metallic"]
            )
        ...
        if not principled_bsdf.inputs["Base Color"].is_linked:
            ...
            mix_node.location = (
                separate_rgb.location[0] + 200,
                separate_rgb.location[1],
            )
            ...
            if "Color2" in mix_node.inputs:
                node_tree.links.new(
                    separate_rgb.outputs["R"], mix_node.inputs["Color2"]
                )
            ...
            if "Result" in mix_node.outputs:
                node_tree.links.new(
                    mix_node.outputs["Result"], principled_bsdf.inputs["Base Color"]
                )
                print(f"DEBUG: Connected Result to Base Color")
            else:
                node_tree.links.new(
                    mix_node.outputs[0], principled_bsdf.inputs["Base Color"]
                )
...
def _create_material_from_textures(
    ...
    try:
        ...
        if not principled_bsdf:
            error_msg = (
                f"Could not find Principled BSDF in material {new_material.name}"
            )
        ...
        for texture_type, image in texture_items:
            print(
                f"DEBUG: Processing texture_type: '{texture_type}', image: {image.name}"
            )
        ...
        print(
            f"DEBUG: valid_items before sorting: {[(item[0], item[2].get('priority', 999)) for item in valid_items]}"
        )
        valid_items.sort(key=lambda x: x[2].get("priority", 999))
        print(
            f"DEBUG: valid_items after sorting: {[(item[0], item[2].get('priority', 999)) for item in valid_items]}"
        )
        ...
        for i, (texture_type, image, socket_info) in enumerate(valid_items):
            ...
            print(
                f"DEBUG: Available BSDF inputs: {list(principled_bsdf.inputs.keys())}"
            )
            ...
            # Special handling for normal maps
                ...
                node_tree.links.new(
                    tex_node.outputs["Color"], normal_node.inputs["Color"]
                )
                node_tree.links.new(
                    normal_node.outputs["Normal"], principled_bsdf.inputs[socket_name]
                )
            ...
            # Special handling for textures that should mix with base color
                ...
                if "Color2" in mix_node.inputs:
                    node_tree.links.new(
                        tex_node.outputs["Color"], mix_node.inputs["Color2"]
                    )
                ...
                print(
                    f"DEBUG: principled_bsdf.inputs keys: {list(principled_bsdf.inputs.keys())}"
                )

                if (
                    output_socket in tex_node.outputs
                    and socket_name in principled_bsdf.inputs
                ):

                    # Check if socket is already connected (avoid conflicts)
                    if not principled_bsdf.inputs[socket_name].is_linked:
                        node_tree.links.new(
                            tex_node.outputs[output_socket],
                            principled_bsdf.inputs[socket_name],
                        )
        ...
        if base_color_mixers:
            ...
            for texture_type, image, socket_info in valid_items:
                print(
                    f"DEBUG: Checking for diffuse texture: {texture_type}, socket: {socket_info['socket']}, mix_with_base: {socket_info.get('mix_with_base', False)}"
                )
                if (
                    socket_info["socket"] == "Base Color"
                    and not socket_info.get("mix_with_base", False)
                    and not socket_info.get("is_packed_texture", False)
                ):
            ...
            for j, (mix_node, tex_node) in enumerate(base_color_mixers):
                print(
                    f"DEBUG: Processing base color mixer {j+1}/{len(base_color_mixers)}"
                )
                if current_input:
                    print(f"DEBUG: Mix node inputs: {list(mix_node.inputs.keys())}")
            ...
            if current_input and not principled_bsdf.inputs["Base Color"].is_linked:
                ...
                print(
                    f"DEBUG: Not connecting final result - current_input: {current_input}, Base Color linked: {principled_bsdf.inputs['Base Color'].is_linked}"
                )
...
class TextureSetMaterialCreate(b_t.Operator):
    ...
    def execute(self, context: b_t.Context) -> set[str]:
        ...
        for i, texture_props in enumerate(texture_set.textures):
            ...
            print(
                f"DEBUG: Available images in bpy.data.images: {[img.name for img in bpy.data.images]}"
            )
        ...
        for i, mesh_props in enumerate(texture_set.meshes):
            ...
            for j, slot in enumerate(obj.material_slots):
                print(
                    f"DEBUG: Material slot {j}: {slot.material.name if slot.material else 'None'}"
                )
                if slot.material:
            ...
            if obj_materials:
                ...
                print(
                    f"DEBUG: Added {len(obj_materials)} materials for object {obj.name}"
                )
            else:
        ...
        for i, original_material in enumerate(materials_to_process):
            print(
                f"DEBUG: Creating material {i+1}/{len(materials_to_process)}: {original_material.name}"
            )
            try:
        ...
        if self.assign_to_objects and created_materials:
            ...
            for obj_name, obj_materials in objects_materials.items():
                ...
                for slot_index, original_material in enumerate(obj_materials):
                    print(
                        f"DEBUG: Slot {slot_index}, original material: {original_material.name}"
                    )
                    if original_material in created_materials:
                        if slot_index < len(obj.material_slots):
                            old_name = (
                                obj.material_slots[slot_index].material.name
                                if obj.material_slots[slot_index].material
                                else "None"
                            )
                            obj.material_slots[slot_index].material = created_materials[
                                original_material
                            ]
                            new_name = created_materials[original_material].name
                            print(
                                f"DEBUG: Assigned slot {slot_index}: {old_name} -> {new_name}"
                            )
                            assigned_count += 1
                        else:
                            print(
                                f"DEBUG: Slot {slot_index} out of range for object {obj_name}"
                            )
                    else:
                        print(
                            f"DEBUG: No created material found for {original_material.name}"
                        )
            ...
            print(
                f"DEBUG: Skipping material assignment - assign_to_objects: {self.assign_to_objects}, created_materials count: {len(created_materials)}"
            )
